...
(function academyCountdownModule(){
    const container = document.getElementById('academyCountdownContainer');
    ...
    // If endISO has no explicit offset, assume TZ and synthesize an offset using formatter
    if (!/[Zz]|[\\+\\-]\\d{2}:?\\d{2}/.test(endISO)) {
        ...
        // Build end at 23:59:59 in TZ
        const [y,m,d] = endISO.split('T')[0].split('-').map(Number);
        const endLocal = new Date(Date.UTC(y, m-1, d, 23, 59, 59));
        // Adjust so that endLocal represents that wall time in TZ by subtracting the current TZ offset
        const offsetMin = tzNow.offsetMinutes;
        endDate = new Date(endLocal.getTime() - offsetMin * 60 * 1000);
    }

    function getTZNow(tz) {
        // Returns { date: Date, offsetMinutes: number } representing current time in tz
        const d = new Date();
        ...
        // A Date constructed without timezone is local; use UTC to avoid local offset affecting
        const tzMillis = Date.UTC(year, month-1, day, hour, minute, second);
        // Compute current UTC millis independently
    ...
    // Build weekend schedule in dropdown
    if (weekendList) {
        renderWeekendSchedule();
        // Rebuild when dropdown opens to ensure current reference date
    ...
    function renderWeekendSchedule() {
        ...
        while (cursor.getUTCDay() !== 6) { // Saturday=6 (UTC day; but cursor is adjusted to TZ wall time approx)
    ...
    // Expose a minimal API for adjusting end date later if needed
