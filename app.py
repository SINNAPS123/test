...
class ReadOnlyToken(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    token = db.Column(db.String(100), unique=True, nullable=False, default=lambda: secrets.token_urlsafe(32))
    description = db.Column(db.String(255), nullable=True)
    scope = db.Column(db.String(50), nullable=False) # 'admin', 'batalion', 'companie'
    unit_id = db.Column(db.String(50), nullable=True) # e.g., '1' for company 1. Null for admin scope.
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    expires_at = db.Column(db.DateTime, nullable=True) # Null for no expiration
    created_by_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    creator = db.relationship('User', backref=db.backref('read_only_tokens', lazy=True))

    def is_valid(self):
        if self.expires_at and self.expires_at.replace(tzinfo=pytz.UTC) < datetime.now(pytz.UTC):
            return False
        return True

    def __repr__(self):
        return f'<ReadOnlyToken id={self.id} scope={self.scope} unit_id={self.unit_id}>'
...
@app.route('/volunteer/create_from_generated', methods=['POST'])
@login_required
def volunteer_create_activity_from_generated():
    if current_user.role != 'gradat':
        flash('Acces neautorizat.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    if not activity_name:
        flash('Numele activității este obligatoriu.', 'warning')
        # We can't easily redirect back to the generation page with the list,
        # so we redirect to the main volunteer page.
        return redirect(url_for('volunteer_home'))

    if not student_ids:
        flash('Niciun student nu a fost selectat pentru a fi adăugat la activitate.', 'warning')
        return redirect(url_for('volunteer_home'))

    try:
        activity_date_obj = datetime.strptime(activity_date_str, '%Y-%m-%d').date()
    except (ValueError, TypeError):
        flash('Data activității este invalidă. Se va folosi data curentă.', 'warning')
        activity_date_obj = get_localized_now().date()
    ...
    for student_id in student_ids:
        # Verify that student belongs to the current gradat to prevent unauthorized additions
        student = Student.query.filter_by(id=student_id, created_by_user_id=current_user.id).first()
        if student:
            participant = ActivityParticipant(activity=new_activity, student_id=student_id, points_awarded=0)
    ...
    if students_added_count > 0:
        try:
            db.session.commit()
            flash(f'Activitatea "{activity_name}" a fost creată cu succes cu {students_added_count} participanți.', 'success')
            # Redirect to the new activity's detail page to see the participants
            return redirect(url_for('volunteer_activity_details', activity_id=new_activity.id))
        except Exception as e:
            db.session.rollback()
            flash(f'Eroare la salvarea activității: {str(e)}', 'danger')
            return redirect(url_for('volunteer_home'))
    else:
        # This case might happen if student_ids were passed but none belonged to the user
        flash('Niciun participant valid nu a putut fi adăugat.', 'warning')
        return redirect(url_for('volunteer_home'))
...
# --- Bulk Operations Hub ---
@app.route('/gradat/bulk_operations', endpoint='bulk_operations_hub')
@login_required
def bulk_operations_hub():
    if current_user.role != 'gradat':
        flash('Acces neautorizat.', 'danger')
        return redirect(url_for('dashboard'))
    return render_template('bulk_operations_hub.html')


# --- Student Profile Page ---
@app.route('/student/<int:student_id>/profile', endpoint='student_profile')
@login_required
def student_profile(student_id):
    ...
    if current_user.role == 'admin':
        ...
        company_id = _get_commander_unit_id(current_user.username, "CmdC")
        if student.companie == company_id:
            user_can_view = True
    elif current_user.role == 'comandant_batalion':
        battalion_id = _get_commander_unit_id(current_user.username, "CmdB")
        if student.batalion == battalion_id:
            user_can_view = True
    ...
    # For participations, we need to join with VolunteerActivity to sort by activity date
    participations = ActivityParticipant.query.join(VolunteerActivity, ActivityParticipant.activity_id == VolunteerActivity.id)\
        .filter(ActivityParticipant.student_id == student_id)\
        .order_by(VolunteerActivity.activity_date.desc())\
        .options(joinedload(ActivityParticipant.activity)).all()
...
# --- Read-Only Token Routes ---

@app.route('/view/<string:token>')
def view_by_token(token):
    token_obj = ReadOnlyToken.query.filter_by(token=token).first()

    if not token_obj:
        return render_template('token_invalid.html', message="Tokenul de acces nu a fost găsit."), 404

    if not token_obj.is_valid():
        return render_template('token_invalid.html', message="Acest token de acces a expirat."), 403

    students_in_scope = []
    report_title = "Raport General"

    if token_obj.scope == 'admin':
        students_in_scope = Student.query.all()
        report_title = "Raport General (Admin)"
    elif token_obj.scope == 'comandant_batalion':
        students_in_scope = Student.query.filter_by(batalion=token_obj.unit_id).all()
        report_title = f"Raport Batalionul {token_obj.unit_id}"
    elif token_obj.scope == 'comandant_companie':
        students_in_scope = Student.query.filter_by(companie=token_obj.unit_id).all()
        report_title = f"Raport Compania {token_obj.unit_id}"
    else:
        return render_template('token_invalid.html', message="Scopul acestui token este invalid sau neconfigurat."), 500

    if not students_in_scope:
        # Still show the page but with a message that there are no students.
        # Create a dummy situation object to avoid template errors.
        situation_now = {
            "efectiv_control": 0, "efectiv_prezent_total": 0, "efectiv_absent_total": 0,
            "in_formation_count": 0, "on_duty_count": 0, "platoon_graded_duty_count": 0, "smt_count": 0, "exempt_other_count": 0,
            "in_formation_students_details": [], "on_duty_students_details": [], "platoon_graded_duty_students_details": [],
            "absent_students_details": [], "smt_students_details": [], "exempt_other_students_details": [],
            "datetime_checked": get_localized_now()
        }
    else:
        # Calculate situation for the current time
        now_for_check = get_localized_now()
        situation_now_raw = _calculate_presence_data(students_in_scope, now_for_check)
        situation_now = {
            **situation_now_raw,
            "datetime_checked": now_for_check
        }

    return render_template('public_view_dashboard.html',
                           report_title=report_title,
                           token=token_obj,
                           situation_now=situation_now,
                           generated_at=get_localized_now())
...
def manage_tokens():
    if current_user.role not in ['admin', 'comandant_companie', 'comandant_batalion']:
        flash('Acces neautorizat pentru a gestiona tokenuri.', 'danger')
        return redirect(url_for('dashboard'))
    ...
    if request.method == 'POST':
        ...
        if not description:
            flash('Descrierea tokenului este obligatorie.', 'warning')
        ...
        if expires_at_str:
            try:
                ...
                flash('Formatul datei de expirare este invalid.', 'warning')
                return redirect(url_for('manage_tokens'))

        scope = current_user.role
        unit_id = None
        if current_user.role == 'comandant_companie':
            unit_id = _get_commander_unit_id(current_user.username, "CmdC")
        elif current_user.role == 'comandant_batalion':
            unit_id = _get_commander_unit_id(current_user.username, "CmdB")

        new_token = ReadOnlyToken(
        ...
        try:
            db.session.commit()
            flash('Tokenul de acces a fost generat cu succes!', 'success')
        except Exception as e:
            db.session.rollback()
            flash(f'Eroare la generarea tokenului: {str(e)}', 'danger')

        return redirect(url_for('manage_tokens'))

    # GET request
    tokens = ReadOnlyToken.query.filter_by(created_by_user_id=current_user.id).order_by(ReadOnlyToken.created_at.desc()).all()
    return render_template('manage_tokens.html', tokens=tokens)

@app.route('/tokens/revoke/<int:token_id>', methods=['POST'])
@login_required
def revoke_token(token_id):
    token = ReadOnlyToken.query.get_or_404(token_id)
    if token.created_by_user_id != current_user.id and current_user.role != 'admin':
        flash('Nu aveți permisiunea să revocați acest token.', 'danger')
        return redirect(url_for('manage_tokens'))

    try:
        db.session.delete(token)
        db.session.commit()
        flash('Tokenul a fost revocat cu succes.', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'Eroare la revocarea tokenului: {str(e)}', 'danger')
